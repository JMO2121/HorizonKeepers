...
const Message = require('../models/Electricity');
...
router.get('/', async (req, res) => {
    ...
    const messagesWithLevel = messages.map(msg => {
        ...
        const level = user ? user.chatStats.EL.level : 1;
...
router.post('/', async (req, res) => {
    try {
        ...
        user.chatStats.EL.messagesSent += 1;
        user.chatStats.EL.level = Math.floor(user.chatStats.EL.messagesSent / 10) + 1;
        await user.save();

        const message = new Message({ sender, content });
        await message.save();
        res.status(201).json({ ...message.toObject(), level: user.chatStats.EL.level });
    } catch (err) {
        console.error(err);
