...
const Message = require('../models/Healthmessage');
...
router.get('/', async (req, res) => {
    ...
    const messagesWithLevel = messages.map(msg => {
        ...
        const level = user ? user.chatStats.HE.level : 1;
...
router.post('/', async (req, res) => {
    try {
        ...
        user.chatStats.HE.messagesSent += 1;
        user.chatStats.HE.level = Math.floor(user.chatStats.HE.messagesSent / 10) + 1;
        await user.save();

        const message = new Message({ sender, content });
        await message.save();
        res.status(201).json({ ...message.toObject(), level: user.chatStats.HE.level });
    } catch (err) {
        console.error(err);
