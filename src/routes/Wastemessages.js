...
const Message = require('../models/Wastemessage');
...
router.get('/', async (req, res) => {
    ...
    const messagesWithLevel = messages.map(msg => {
        ...
        const level = user ? user.chatStats.waste.level : 1;
...
router.post('/', async (req, res) => {
    try {
        ...
        user.chatStats.waste.messagesSent += 1;
        user.chatStats.waste.level = Math.floor(user.chatStats.waste.messagesSent / 10) + 1;
        await user.save();

        const message = new Message({ sender, content });
        await message.save();
        res.status(201).json({ ...message.toObject(), level: user.chatStats.waste.level });
    } catch (err) {
        console.error(err);
