...
const Message = require('../models/Hidrologymessage');
...
router.get('/', async (req, res) => {
    ...
    const messagesWithLevel = messages.map(msg => {
        ...
        const level = user ? user.chatStats.hidro.level : 1;
...
router.post('/', async (req, res) => {
    try {
        ...
        user.chatStats.hidro.messagesSent += 1;
        user.chatStats.hidro.level = Math.floor(user.chatStats.hidro.messagesSent / 10) + 1;
        await user.save();

        const message = new Message({ sender, content });
        await message.save();
        res.status(201).json({ ...message.toObject(), level: user.chatStats.hidro.level });
    } catch (err) {
        console.error(err);
