...
const Message = require('../models/Pamessage');
...
router.get('/', async (req, res) => {
    ...
    const messagesWithLevel = messages.map(msg => {
        ...
        const level = user ? user.chatStats.PA.level : 1;
...
router.post('/', async (req, res) => {
    try {
        ...
        user.chatStats.PA.messagesSent += 1;
        user.chatStats.PA.level = Math.floor(user.chatStats.PA.messagesSent / 10) + 1;
        await user.save();

        const message = new Message({ sender, content });
        await message.save();
        res.status(201).json({ ...message.toObject(), level: user.chatStats.PA.level });
    } catch (err) {
        console.error(err);
